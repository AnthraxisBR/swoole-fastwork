...
class Zone extends \AnthraxisBR\SwooleFW\CloudServices\GCP\FwGoogleClient
{
	public function OperationsDelete()
	{
		...
		$args[] = "{zone}";
		$args[] = "{resourceId}";
		$url = $this->replaceUri('https://www.googleapis.com/compute/v1/projects/{project}/zones/{zone}/operations/{resourceId}', $args);
		$queryArgs = [];
	...
	public function OperationsList(int $maxResults, string $pageToken, string $filter, string $orderBy)
	{
		...
		$args[] = "{project}";
		$args[] = "{zone}";
		$url = $this->replaceUri('https://www.googleapis.com/compute/v1/projects/{project}/zones/{zone}/operations', $args);
		$queryArgs = [];
	...
	public function Get()
	{
		...
		$args[] = "{project}";
		$args[] = "{resourceId}";
		$url = $this->replaceUri('https://www.googleapis.com/compute/v1/projects/{project}/zones/{resourceId}', $args);
		$queryArgs = [];
		$url = $this->parseArgs($url, $queryArgs);
		$url = $this->prepareUrl($url);
		return $this->get($url);
	...
	public function List(int $maxResults, string $pageToken, string $filter, string $orderBy)
	{
		$args = [];
		$args[] = "{project}";
		$url = $this->replaceUri('https://www.googleapis.com/compute/v1/projects/{project}/zones', $args);
		$queryArgs = [];
		$queryArgs[] = $maxResults;
		$queryArgs[] = $pageToken;
		$queryArgs[] = $filter;
